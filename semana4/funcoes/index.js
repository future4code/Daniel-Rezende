...
/* 1 a - será impresso respectivamente: 10 e 50
b - se os consoles.log fossem retirados nao apareceria mais nada

2 a - Darvas e Caio, ja que o laço se repetirá apenas 2 vezes
b - Amanda e Caio

3 - cria a função metodo e a variavel arrayFinal, cria a variavel de laço X que irá percorrer todo o array, sendo que se o x for par, o arrayFinal pegará esse valor par do x multiplicado por ele mesmo. No final a função deixa o arrayFinal disponivel para ser acessado fora da função através do return. Eu daria para essa função o nome de "verificarNumAtribuirArray"
*/
...
function informacaoUsuario(nome , idade, cidade, estudante){
    ...
    else{console.log("digite uma alternativa valida para estudante(true ou false)")}
}
informacaoUsuario("daniel", 19, "belem", true)

function soma(num1, num2){
    resultado = num1 + num2
    return resultado
}
console.log(soma(50,31))

function verificarRelacaoNum(num1, num2){
    let resultado 
    if (num1 >= num2){
        resultado = true
    }
...
function devolverResultadoArray(array){
    return array.length
}
console.log(devolverResultadoArray([10, 23, 45, 78, 90, 52, 35, 67, 84, 22]))

function verificarNumParImpar(numero){
    if(numero % 2 === 0){
       resultado = true
    }
    else if(numero % 2 !== 0){
...
console.log(verificarNumParImpar(10))

function devolverParesArray(array){
    let resultado = 0
...
imprimirParametro("Seja bem vindo ao desafio extra")

let operacaoValores = (val1, val2) => {
    resultado = val1 + val2
    imprimirParametro(resultado)
}
operacaoValores(53, 11)

function retornarValoresParesx2(array){
    let resultado = []
    for(contador = 0;contador <= array.length;contador++){
        laco = array[contador]
        if(laco % 2 == 0){
            resultado.push(laco * 2)
        }
    }
    return resultado
}
console.log(retornarValoresParesx2([0, 8, 23, 16, 10, 15, 41, 12, 13]))

function retornarMaiorNumeroEIndice(array){
    let maiorNumero = 0
...
console.log(retornarMaiorNumeroEIndice([0, 8, 23, 16, 10, 15, 41, 12, 13]))

function inverterArray(array){
    let listaInvertida = []
    for(contador = array.length -1;contador >= 0;contador--){
        laco = array[contador]
        listaInvertida.push(laco)
    }
    
    return listaInvertida
}
console.log(inverterArray([0, 8, 23, 16, 10, 15, 41, 12, 13]))